// import 'package:chat_pro/analyse.dart';
...
//                               value: '当前模型',
//                               child: Text(
//                                   '当前模型: ${OpenAIUserInteraction().model}'),
//                               onTap: () {
...
import 'package:chat_pro/analyse.dart';
import 'package:chat_pro/backend.dart';
import 'package:chat_pro/chat_controller.dart' as chatController; // 添加前缀
import 'package:chat_pro/chat_list.dart';
import 'package:chat_pro/ui/pura_multiple_radial_gradients.dart';
import 'package:chat_pro/ui/theme.dart';
import 'package:chinese_font_library/chinese_font_library.dart';
...
void main() async {
  ...
  try {
    await OpenAIUserInteraction().init();
  } catch (e, stackTrace) {
    print('OpenAI 初始化失败: $e');
    Logger.logError('OpenAI 初始化失败: $e', stackTrace);
  }
  runApp(const MainApp());
}
    ...
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(
            create: (_) => chatController.ChatController()), // 使用前缀
        ChangeNotifierProvider(create: (_) => ImagesBoardManager()),
      ],
      child: MaterialApp(
        debugShowCheckedModeBanner: false,
        ...
        home: Stack(
          children: [
            Selector<chatController.ChatController, String>(
              // 使用前缀
              selector: (_, myType) => ChatThemes().currentTheme,
              shouldRebuild: (previous, next) => true,
              builder: (context, currentTheme, child) {
